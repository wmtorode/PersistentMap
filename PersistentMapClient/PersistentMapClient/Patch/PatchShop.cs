...
namespace PersistentMapClient {
    ...
    public static class SG_Shop_Screen_ChangeToStoreTypeState_Patch {
        static void Prefix(SG_Shop_Screen __instance, SG_Shop_Screen.StoreType newType, Shop.ShopType ___shopType, StarSystem ___theSystem, SimGameState ___simState) {
            try {
                if (newType == SG_Shop_Screen.StoreType.FactionStore && ___simState.IsFactionAlly(___theSystem.OwnerValue, null)) {
    ...
    //        try {
    //            if (__instance.ThisShopType == Shop.ShopType.Faction && ___Sim.IsFactionAlly(___system.OwnerValue, null)) {
    //                __instance.Clear();
    //                if (!Fields.LastUpdate.ContainsKey(___system.OwnerValue.Name)) {
    //                    Fields.LastUpdate.Add(___system.OwnerValue.Name, DateTime.MinValue);
    //                }
